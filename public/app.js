...
let plan=null,session='',selected=0,single=false,busy=false,editing=null,editVersion=0,noticeTimer;
...
function render(){
  ...
  document.querySelectorAll('[data-day]').forEach(b=>b.onclick=()=>{selected=Number(b.dataset.day);render();});
  ...
  $('board').innerHTML=days.map((day,i)=>{
    ...
    return `<section class="day ${now.day===i?'current':''} ${selected===i?'active':''}" data-drop="${i}"><div class="day-head"><h3>${day}</h3>${now.day===i?'<span class="today-tag">DZIŚ</span>':`<span class="day-count">${events.length} zajęć</span>`}</div>${events.map(e=>{let label='';if(e.start>='16:30'&&!afternoon){afternoon=true;label='<p class="section-label">Po lekcjach</p>';}
      const tag=session?'button':'article';return label+`<${tag} class="event ${e.kind} ${session?'editable':''} ${e.day===now.day&&e.start<=now.time&&e.end>now.time?'live':''}" data-id="${esc(e.id)}" ${session?'draggable="true"':''}>${e.uncertain?'<span class="uncertain" title="Do potwierdzenia">?</span>':''}<span class="time">${e.start}${e.end?' — '+e.end:''}</span><span class="name">${esc(e.name)}</span>${e.note?`<span class="note">${esc(e.note)}</span>`:''}</${tag}>`;
...
async function save(events,version){
  if(busy)throw Error('Poczekaj na zakończenie zapisu.');busy=true;
  try{plan=await request('/api/plan','PUT',{events,version});render();notice('Zapisano. Zmiana będzie widoczna u wszystkich.');}
  catch(e){if(e.status===409){await load();editVersion=plan.version;}if(e.status===403){session='';render();}throw e;}
  finally{busy=false;}
}
...
function openEvent(id){
  ...
  const e=plan.events.find(x=>x.id===id)||{name:'',day:selected,start:'08:00',end:'08:45',kind:'edu',note:'',uncertain:false},f=$('event-form');
  for(const key of ['name','day','start','end','kind','note'])f.elements[key].value=e[key];f.elements.uncertain.checked=e.uncertain;
...
$('export').onclick=()=>{const blob=new Blob([JSON.stringify({events:plan.events},null,2)],{type:'application/json'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='plan-lekcji.json';a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);};
$('import').onclick=()=>$('file').click();$('file').onchange=async e=>{const f=e.target.files[0];if(!f)return;try{if(f.size>100000)throw Error('Plik jest za duży.');const data=JSON.parse(await f.text());if(!Array.isArray(data.events))throw Error('Plik nie zawiera kopii planu.');if(await confirmAction('Zastąpić cały plan?',`Wczytasz ${data.events.length} wpisów. Obecny plan zostanie zastąpiony dla wszystkich.`))await save(data.events,plan.version);}catch(err){notice(errorText(err));}finally{e.target.value='';}};
